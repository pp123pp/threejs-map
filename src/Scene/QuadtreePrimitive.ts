...
function processSinglePriorityLoadQueue (
    primitive:QuadtreePrimitive,
    frameState: FrameState,
    tileProvider: GlobeSurfaceTileProvider,
    endTime: number,
    loadQueue: QuadtreeTile[],
    didSomeLoading: boolean
) {
...
const selectTilesForRendering = function selectTilesForRendering (primitive: QuadtreePrimitive, frameState: FrameState) {
    const debug = primitive._debug;
    ...
    let tile;
    const levelZeroTiles = primitive._levelZeroTiles;
    const occluders: any = levelZeroTiles.length > 1
