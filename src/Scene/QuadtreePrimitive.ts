 ...
 * @private
 */
function updateTileLoadProgress (primitive: any, frameState: FrameState) {
    ...
    if (debug.enableDebugOutput && !debug.suspendLodUpdate) {
        debug.maxDepth = primitive._tilesToRender.reduce(function (max: any, tile: any) {
...
class QuadtreePrimitive {
    ...
    get tileLoadProgressEvent (): Event {
 ...
 */
     ...
     *        function is passed a reference to the tile as its only parameter.
     */
    forEachRenderedTile (tileFunction: any): any {
...
class TraversalDetails {
    allAreRenderable: boolean;
    anyWereRenderedLastFrame: boolean;
    notYetRenderableCount: number;
    constructor () {
        /**
...
class TraversalQuadDetails {
    ...
    combine (result: TraversalDetails) {
...
function visitTile (
    ...
    frameState: any,
    tile: any,
    ancestorMeetsSse: any,
    traversalDetails: any
) {
    const debug = primitive._debug;
...
function updateHeights (primitive: any, frameState: FrameState) {
