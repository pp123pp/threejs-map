...
const scratchClippingPlanesMatrix = new CesiumMatrix4();
const scratchInverseTransposeClippingPlanesMatrix = new CesiumMatrix4();
...
const addDrawCommandsForTile = (tileProvider: GlobeSurfaceTileProvider, tile: any, frameState: FrameState) => {
    ...
    maxTextures -= (globeTranslucencyState as GlobeTranslucencyState).numberOfTextureUniforms;
    const mesh = surfaceTile.renderedMesh;
    let rtc = mesh.center;
    const encoding = mesh.encoding as TerrainEncoding;
    const tileBoundingRegion = surfaceTile.tileBoundingRegion;
    ...
    if (materialUniformMapChanged) {
        // tileProvider. = tileProvider.materialUniformMap;
    ...
    do {
        ...
        if (tileProvider._drawCommands.length <= tileProvider._usedDrawCommands) {
            ...
            command.material = new GlobeSurfaceTileMaterial();
        ...
        uniformMapProperties.center3D.set(meshCenter.x, meshCenter.y, meshCenter.z);
        ...
        while (numberOfDayTextures < maxTextures && imageryIndex < imageryLen) {
            const tileImagery = tileImageryCollection[imageryIndex] as TileImagery;
            const imagery = tileImagery.readyImagery as Imagery;
            ++imageryIndex;
        ...
        let boundingVolume = command.boundingVolume as BoundingSphere;
